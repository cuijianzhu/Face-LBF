	...
	cv::Mat pixels_val, cv::Mat pixels_cov)
{
	...
	for (int i = 0; i < training_parameters.F; ++i)
	{
		...
		static_cast<cv::Mat>(Y * projection).copyTo(Y_proj);	// Y_proj: projecting Mat Y to the random vector, N*1
		double Y_proj_cov = Covariance(Y_proj.ptr<double>(0),	
			Y_proj.ptr<double>(0), Y_proj.total());				// Y_proj_cov: Variance of Y_proj, double
		vector<double> Y_pixels_cov(pixels_val.rows);			// Y_pixels_cov: Covariances of pixel with Yproj, P*1
	...
	int outputs_count = 1 << training_parameters.F;	// 2^F
	outputs.assign(outputs_count, vector<cv::Point2d>((*targets)[0].size())); // mapping table of (2^F) * (2*landmarks)
	vector<int> each_output_count(outputs_count);	// the number of the vectors falls in each subspace
	...
	for (int i = 0; i < outputs_count; ++i)
	{
		for (cv::Point2d &p : outputs[i])
			p *= 1.0 / (each_output_count[i] + training_parameters.Beta);	// Beta: to diminish the influence?
	}
}
